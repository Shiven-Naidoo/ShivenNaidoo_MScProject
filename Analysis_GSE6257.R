...
untar("GSE????/GSE????_RAW.tar", exdir = "data/")
...
## INCOMPLETE!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
#!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
## 3. call and store raw data from GEO in an object

raw_data_2 = getGEO(sample_ID)

## In some cases, your micrarray data might contain multiple series. 
## Identify and isolate your series of interest
## For example, for the GSE6257 dataset, the first series is looking at microarray data for
## a mouse sample whereas the second series is looking at Homo Sapiens

## Browse your data as follows

raw_data_2$`GSE6257-GPL1261_series_matrix.txt.gz`@featureData@data
raw_data_2$`GSE6257-GPL570_series_matrix.txt.gz`@featureData@data

raw_data_2 = raw_data_2[[2]]


## (if you are using method 3, use the code below to get sample info) 
## identify useful column names using pData

sample_info = pData(raw_data)
...
sample_info$Experiment = c("HDLEC_control 1", "HDLEC_control 2", "HDLEC_control 3", "HDLEC_TNF-stimulated 1", "HDLEC_TNF-stimulated 2", "HDLEC_TNF-stimulated 3")
...
GEO_anno_data = GEO_series_data$`GSE6257-GPL570_series_matrix.txt.gz`@featureData@data # isolates data file which includes gene symbols in column x
...
write.table(rma_normalised_annotated_expr, file = "expr_data_GSE6257", sep = "\t", quote = FALSE, row.names = TRUE, col.names = TRUE)
...
## for dataset GSE6257, control 1 and treatment 1 show divergence from the other groups along PCA 1
## This could be due to technical error and requires further investigation
## Since the cells are cultured from tissue samples of different patients, they could be 
## caused by differences in the gene expression of each patient
